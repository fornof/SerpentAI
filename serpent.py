...
def setup_base():
    ...
    os.makedirs(os.path.join(os.getcwd(), dir_mod+"datasets/collect_frames"), exist_ok=True)
    os.makedirs(os.path.join(os.getcwd(), dir_mod+"datasets/collect_frames_for_context"), exist_ok=True)
    os.makedirs(os.path.join(os.getcwd(), dir_mod+"datasets/current"), exist_ok=True)
...
def object_recognition(game_agent_name, model_name):
    global dir_mod
    model_path = f"{dir_mod}plugins/{game_agent_name}Plugin/files/ml_models/object_recognition/{model_name}"
...
def prepare_game_plugin(game_name, game_platform):
    ...
    shutil.copytree(
        os.path.join(os.path.dirname(__file__), dir_mod+"templates/SerpentGamePlugin".replace("/", os.sep)),
...
def prepare_game_agent_plugin(game_agent_name):
    plugin_destination_path = f"{dir_mod}{offshoot.config['file_paths']['plugins']}/Serpent{game_agent_name}GameAgentPlugin".replace("/", os.sep)

    shutil.copytree(
        os.path.join(os.path.dirname(__file__), dir_mod+"templates/SerpentGameAgentPlugin".replace("/", os.sep)),
        plugin_destination_path
    )
